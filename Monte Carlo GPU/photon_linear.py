...
def polarized_photon_linear(seed, incident_degree, ne, reflection, zstokes, random_nums, U, W, jones, mu_as,
                            mu_ss, positions, s1, s2, m11, m12, scat_events,
                            jones_partial, co_xy,
                            cross_xy, incoh_cross_xy, co_rz, cross_rz, incoh_cross_rz, co_xy_trad, cross_xy_trad,
                            incoh_cross_xy_trad, co_rz_trad, cross_rz_trad, incoh_cross_rz_trad, rng_states,
                            thread_idx):
    ...
    for step in range(steps):
        ...
        if scat_event == 0:
            # U_new[0] = cmath.sin(deg).real
            # U_new[1] = 0
            # U_new[2] = cmath.cos(deg).real

            # U_epar[0] = cmath.cos(deg).real
            # U_epar[1] = 0
            # U_epar[2] = cmath.sin(deg).real
            U_new[0] = 0
            U_new[1] = 0
            U_new[2] = 1
            U_epar[0] = 1
...
def process_steps_linear(seed, incident_degree, ne, reflection, zstokes, rng_states, modes, random_nums, U, W,
                         jones, mu_as, mu_ss, scat_events,
                         jones_partial, co_xy, cross_xy, incoh_cross_xy, co_rz, cross_rz, incoh_cross_rz, co_xy_trad,
                         cross_xy_trad, incoh_cross_xy_trad,
    ...
    if modes[thread_idx] == 3:
        polarized_photon_linear(seed, incident_degree, ne,reflection, zstokes, random_nums[thread_idx], U, W,
                                jones, mu_as, mu_ss, positions, s1, s2, m11, m12,
